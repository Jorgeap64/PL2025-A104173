...
def load_products():
    try:
        with open("TPC5/vending.json", "r") as file:
            data = json.load(file)
        table = PrettyTable()
...
def save_products(stock):
    try:
        with open("TPC5/vending.json", "w") as file:
            json.dump(stock, file, indent=4)
        return True
    except Exception as e:
        print(f"[DEBUG] ERRO ao alterar o ficheiro vending.json. {e}")
        return False
        ...
        if item["cod"] == code:
            if item["quant"] > 0:
                ...
                if temp < 0:
                    ...
                    item["quant"] -= 1
                    if save_products(stock):
                        print(f'Pode retirar o produto dispensado. {item["nome"]}')
                        return temp
                    else:
                        print(f'[MÁQ.] ERRO ao atualizar o stock. Operação cancelada.')
                        item["quant"] += 1
                        return saldo
            else:
...
def machine():
    ...
    if stock is None:
        print("[MÁQ.] ERRO ao carregar o stock. A sair...")
        return
    ...
    while True:
        try:
            ...
            print(f'[MÀQ.] Ocorreu um erro. Tente novamente. {str(e)}')
